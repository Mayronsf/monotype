...
export default function MatchResult({ groupId, round, matchIndex, player1, player2 }) {
  const { updateResult, removeResult, getResult, error } = useChampionship();
  const [pokemonDiff, setPokemonDiff] = useState('');
  const [winner, setWinner] = useState(null);
  const [isSaving, setIsSaving] = useState(false);
  const [saveError, setSaveError] = useState(null);
  
  const existingResult = getResult(groupId, round, matchIndex);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await updateResult(groupId, round, matchIndex, winnerId, loserId, diff);
  ...
  const handleEdit = () => {
    if (existingResult) {
      ...
      // Não resetamos pokemonDiff aqui, deixamos o usuário editar
    }
  };

  const handleReset = () => {
    if (confirm('Tem certeza que deseja remover este resultado?')) {
      removeResult(groupId, round, matchIndex);
      setWinner(null);
      setPokemonDiff('');
    }
  };
  ...
  return (
    <div className="w-full bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-4 border-2 border-gray-300 hover:border-blue-500 transition-all duration-300">
